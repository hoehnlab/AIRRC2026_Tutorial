...
To run BEAST2 with Dowser, you will need to specify the path to the BEAST2 executable and the path to the XML template that you want to use. The XML template will specify the model and priors that you want to use for your analysis. Default templates are available at github.com/hoehnlab/xml-templates, but you can also create your own if you want to specify different models or priors.

```{r}
beast <- "/opt/beast/bin/beast"
strict_template <- "data/xml-templates/template.xml"
tyche_template <- "data/xml-templates/template.xml"
```
...
the clock rate using a using root-to-tip regression or by fitting a BEAST2 strict clock 
model to GC B cells. 
...
mixed_trees <- readRDS("mixed_trees.RDS")
```
