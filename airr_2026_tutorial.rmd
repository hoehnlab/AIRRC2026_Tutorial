...
<!-- You will need to have BEAST2 installed and the executable path for BEAST2 specified in the `beast` argument of the `getTimeTrees` function. You will also need to specify the path to the BEAST2 XML template in the `template` argument. -->
...
allow us to specify a germline sequence and use a clock model that accounts for the heterogenous rates of evolution across B cell subtypes. 
...
```{r}
beast <- "/path/to/beast2/executable"
strict_template <- "/path/to/xml/template.xml"
tyche_template <- "/path/to/xml/template.xml"
```
...
For this analysis, we will be using the same data as before, but we will need to filter it to include only the heavy chains. 
The column containing the timepoint information needs to be numeric, so we convert
that too. 
```{r, warning=FALSE}
...
The next step takes about 5 minutes to run, depending on your machine, as a
quick demonstration. Expect results to be unconverged. To get more reliable results, set larger `mcmc_length` or `iterations`.

`getTimeTreesIterate` will run BEAST2 on each clone in parallel 
(here, `nproc=1`, so 1 clone at a time, since we only have one!). 
...
MCMC samples (here, `1e5`), and it will repeat this up to `iterations` times (here, `5`), 
so here we have a maximum of 5e5 MCMC samples, which is far too few to expect convergence.
...
The next step takes about 10 minutes to run, depending on your machine, as a
quick demonstration. Expect results to be unconverged. To get more reliable results, set larger `mcmc_length` or `iterations`.
...
chronic infections or repeated vaccinations, you may want to allow transitions in both directions, so you would set this to `"1 1"`. Note: traits are sorted ASCII alphabetically.
...
You can monitor the progress of BEAST2 runs using tools like Tracer and FigTree.
Let's step away from R and take a look at those.
...
require a larger scale bar. In this case, we know the data spans 100 days,
so we set `scale=100` to make the scale bar more visually interpretable.
...
We assumed that the example above was a primary GC. TyCHE also works in cases where you expect
recall GCs or other GC re-entry. To see examples of
other scenarios and how you might change your parameters to fit those scenarios,
